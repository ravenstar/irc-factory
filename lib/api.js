...
Api.prototype.fork = function(exit, options) {
	...
	var logStream = fs.createWriteStream('./fork.log', {flags: 'a'});
...
Api.prototype.setupServer = function(options) {
	...
	this._rpc.on('loadmod', function(key, moduleName) {
		if (!_.has(Clients, key)) {
			return false;
		}
		var client = Clients[key];
		...
		console.info("[RPC] loadmod() %s: (%s) ", key, moduleName)
		Client.prototype['loadModule'].call(client.irc, params);
		// return Client.prototype.loadModule(key, moduleName)
		return true;
