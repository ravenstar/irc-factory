...
Client.prototype.loadModule = function(mod) {
	var self = this; // this == client.irc
	var vm = require('vm'),
	fs = require('fs'),
	mPath = process.env.PWD+'/modules/'+mod+'.js',
	script;

	console.log("[%s] Loading module:", self.key, mod);
	try {
		script = fs.readFileSync(mPath);
	} catch (e) {
		// if (!script) {
		console.error("[%s] Error loading module %s: %s. skipping...", self.key, mod, e.code);
		return false;
	}

	var logConsole = function() {
		data = _.map(Array.prototype.slice.call(arguments, 0)[0], function(v, k) {
			return v;
		})
		data.unshift('['+ mod+']');
		data.push('\r\n');

		fs.appendFileSync(process.env.PWD+'/fork.log', data.join(' '), { flags: 'a' });
		return false;
	}

	var sandbox = {
		_zzz: (function(){
			var name = require.resolve('console');
		...
		console: {
			log: function () { logConsole(arguments) },
			info: function () { logConsole(arguments) },
			warn: function () { logConsole(arguments) },
			error: function () { logConsole(arguments) }
		},
		// console: console,
		...
		isChannel: function(data) {
			return (data[0].indexOf(self.supported.channel.types) !== -1) ? true : false;
		},

		listen: function(data, prefixed, onlyChannel, cb) {
			if (typeof prefixed === "undefined" || prefixed === null) {
				prefixed = true;
			}
			if (typeof onlyChannel === "undefined" || onlyChannel === null) {
				onlyChannel = false;
			}

			// self._E = Events.on([self.key, 'privmsg'], function(payload) {
			Events.on([self.key, 'privmsg'], function(payload) {
				msg = payload.message;

				if (onlyChannel && payload.target[0] !== '#') {
					// if (onlyChannel && payload.target[0].indexOf(self.supported.channel.types) !== -1) {

					return false; // NOT a channel msg
				}

				var zFind = msg.indexOf(data);
				if (zFind === -1 || (prefixed && zFind > 0) ) {
					return false; // key-terms not found in msg
				}

				try {
					cb(payload);
				} catch (err) {
					// TODO lokar: should i replace this console.error with logConsole()?
					console.error("[%s] Caught error in module %s: ", self.key, mod, err);
				}
			});

		},

		_listenRegex: function(dataRegex, callback, once, prefixed) {
			/*
			This is a helper function to listen for a given privmsg.
			@param dataRegex - Takes a RegExp from regexFactory.js, which is checked for in the msg.
			*/
			once = !!once;
			if (!_.isRegExp(dataRegex)) {
				console.error("Error in module %s: listen() does not use a RegExp object. Use regexFactory.", mod);
				return;
			}
			if (typeof prefixed === "undefined" || prefixed === null) {
				prefixed = true;
			...
			self.hookEvent(key, 'privmsg', function(data) {
				match = dataRegex.exec(data.message);
				if (!match) return false;

				try {
					callback(data);
				} catch (err) {
					console.error("[Caught error in module %s]: ", mod, err);
				}

				if (once) {
					console.warn("[%s]TODO: delete 'once' listeners!", mod);
				}
			}, false);
		},

		hook: function(e, callback, once) {
			Events.on([self.key, e], callback, once);
		},

		unhook: function(e, callback) {
			self.unhookEvent(key, e, callback);
		}
	}

	try {
		vm.runInNewContext(script, sandbox, mod);
		console.log("[%s] Loaded "+mod+"!", self.key);
	} catch (err) {
		console.error("[%s] Error while loading module", self.key, mod, '!! ', err);
	}

	// console.log("FIN loading "+mod);
