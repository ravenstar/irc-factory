...
Client.prototype.loadModule = function(mod) {
	...
	// mPath = '/home/lokar/Code/iWizard/modules/'+mod+'.js';
	script = fs.readFileSync(mPath);

	if (!script) {
		console.error("Error while loading module:", mPath);
		return false;
	}
	...
	var sandbox = {
		...
		hook: function(e, callback, once) {
			self.hookEvent(key, e, callback, once);
		},
	...
	try {
		...
		console.error("--> Error while loading module", mod, '!! ', err);
	}
